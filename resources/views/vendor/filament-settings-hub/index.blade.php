<x-filament-panels::page>
    @php
        $settings = \TomatoPHP\FilamentSettingsHub\Facades\FilamentSettingsHub::load()
            ->sortBy('order')
            ->groupBy('group');
    ...
    @foreach ($settings as $settingGroup => $setting)
        <h1 class="text-lg font-bold tracking-tight md:text-3xl filament-header-heading">
            {{ str($settingGroup)->contains(['.', '::']) ? trans($settingGroup) : $settingGroup }}</h1>
            ...
            @foreach ($setting as $item)
                @if ($item->route)
                    @if (filament('filament-settings-hub')->isShieldAllowed())
                        ...
                        @if (
                            $page &&
                                \Filament\Facades\Filament::auth()->user()->can('page_' . $page))
                            <a href="{{ $tenant ? route($item->route, $tenant) : route($item->route) }}"
                                class="fi-ta-ctn flex flex-col justify-center items-center gap-2 p-4 overflow-hidden rounded-xl bg-white shadow-sm ring-1 ring-gray-950/5 dark:divide-white/10 dark:bg-gray-900 dark:ring-white/10">
                                <div class="p-2">
                                    @if (isset($item->icon))
                                        @if ($item->color)
                    ...
                    @else
                        <a href="{{ $tenant ? route($item->route, $tenant) : route($item->route) }}"
                            ...
                            <h1 class="font-bold text-lg">
                                {{ str($item->label)->contains(['.', '::']) ? trans($item->label) : $item->label }}</h1>
                            <p class="text-sm text-gray-400">
                                {{ str($item->description)->contains(['.', '::']) ? trans($item->description) : $item->description }}
                            </p>
                        </a>
